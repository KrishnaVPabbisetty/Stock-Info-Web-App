...
class StockCard extends Component {
    ...
    info = async () => {
        ...
        let response = await axios.get(
          `https://cloud.iexapis.com/stable/stock/${this.props.data.symbol}/chart/dynamic?token=pk_f2b4f7031e85444588089fc987f9e65e`
        );

        // console.log(response.data);
        Modal.success({
            title: `Stock Details for ${this.props.data.name} (${this.props.data.symbol})`,
            width: 700,
            content: (
    ...
    render() {
        ...
        return (
            <div>
                ...
                &&
                <Col span={8}>
                    <Card title={props.name} bordered={false} className="stockCard" extra={<p>{props.symbol}</p>}
                          onClick={this.info}
                          hoverable
                          style={cardStyle}>
                        <Text strong>Price: </Text>{props.price} $
                        <br/>
                        ...
                        <Text style={{fontSize: 10}}>( {(props.changesPercentage * 100).toFixed(2)} % )</Text>
                        <br/>
